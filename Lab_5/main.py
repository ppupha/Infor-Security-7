...
def make_signature(message_file, key):
    ...
    signature_file_name = SIGN_FILE #input("signature filename:")
    with open(signature_file_name, "wb") as f:
        f.write(signature)

    print("Saved signature to file {}".format(SIGN_FILE))
...
def main():
    ...
    (privkey, pubkey) = rsa.newkeys(1024)
    signature_file = make_signature(message_file, privkey)
    filename = SIGN_FILE    #input("file with signature:")
